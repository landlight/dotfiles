...
1. Fork this repo
1. Open a terminal/shell and do this (change `austinrivas` and `master` as appropriate):

#### Ubuntu

```sh
export DOTFILES_GH_USER=austinrivas
...
```sh
bash -c "$(wget -qO- https://github.com/austinrivas/dotfiles.git)" && source ~/.bashrc
```

#### macOS

```sh
bash -c "$(curl -fsSL https://github.com/austinrivas/dotfiles.git)" && source ~/.bashrc
```
